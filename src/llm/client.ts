...
export async function generateDocs(
  ...
  const apiKey = process.env.DOCWEAVER_API_KEY || '';
  if (!apiKey) {
    throw new Error(
  ...
  const baseURL = process.env.DOCWEAVER_BASE_URL || 'https://api.openai.com/v1';
  const model = process.env.DOCWEAVER_MODEL || 'gpt-4o-mini';
  ...
  try {
    ...
    if (typeof error.message === 'string' && error.message.includes('context_length') || error.status === 400) {
